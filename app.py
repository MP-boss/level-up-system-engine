...
# --- THE GAME INTERFACE ---
 ...
 # --- TAB 1: TRAINING ZONE ---
    with tab1:
        ...
        total_done = sum([
            get_reps(log['caption']) 
            for log in st.session_state.gallery_logs 
            if "Push-ups" in log['caption'] and today in log['date']
        ])
        
        # UI for the Quest
        st.info(f"Daily Goal: Complete {quest_target} {quest_ex} today! (Progress: {total_done}/{quest_target})")
        
        # Check if completed and not yet rewarded
        if total_done >= quest_target:
            if "quest_claimed" not in st.session_state:
                st.session_state.gold += 50
                st.session_state.quest_claimed = True
                st.success("Quest Complete: You've earned a bonus +50 Gold!")
                st.balloons()
                save_game() # Added this to ensure the gold saves!
            else:
                st.success("Quest already claimed for today!")
        
        st.subheader("Training Zone")
        st.write(f"### Current System Difficulty: Level {st.session_state.level}")
        
        # Define the library
        EXERCISE_LIBRARY = {
            "Yoga": ["Tadasana", "Suryanamaskar", "Virabhadrasana", "Vajrasana"],
            "Breathing": ["Tratak", "Anulom Vilom", "Kapalbhati", "Bhramari Pranayama"],
            "Strength/Bodyweight": ["Hanging", "Deep Squat Hold", "Push-ups", "Finger Push-ups", "Squats", "Plank", "Towel Squeeze"],
            "Combat": ["Punching", "Kicking"]
        }

        # Scaling logic
        def process_training(ex_name, category):
            # Base rewards
            xp_base = 30
            gold_base = 50
            
            # --- DYNAMIC MULTIPLIER (Legendary Scaling) ---
            if st.session_state.level <= 10:
                multiplier = 1 + (st.session_state.level * 0.1)
            elif st.session_state.level <= 20:
                multiplier = 2 + (st.session_state.level - 10) * 0.5
            else:
                multiplier = 7 + (st.session_state.level - 20) * 1.5
            # -----------------------------------------------

            final_xp = int(xp_base * multiplier)
            final_gold = int(gold_base * multiplier)
            
            st.session_state.xp += final_xp
            st.session_state.gold += final_gold
            
            # Stat gain increases with level (1 point early, 2 points later)
            stat_gain = 1 if st.session_state.level < 10 else 2
            stat_target = 'strength' if category == "Combat" else 'agility'
            st.session_state[stat_target] += stat_gain
            
            # Level Up Logic (Difficulty scales up)
            next_level_xp = st.session_state.level * 150
            if st.session_state.xp >= next_level_xp:
                st.session_state.xp -= next_level_xp
                st.session_state.level += 1
                st.balloons()
                st.toast(f"Level Up! You are now Level {st.session_state.level}!", icon="🔥")
            
            save_game()
            st.success(f"Completed {ex_name}! Gained {final_xp} XP and {final_gold} Gold.")
            st.rerun()

   # Display categories
        for category, exercises in EXERCISE_LIBRARY.items():
            # Only show specific exercises if we are in certain categories
            filtered_exercises = exercises
            if category == "Combat":
                filtered_exercises = ["Punching", "Kicking"]
            ...
            with st.expander(f"📚 {category}"):
                for ex in filtered_exercises:
                    ...
                    if st.session_state.level < 5:
                        ...
                        level_multiplier = 1 + (st.session_state.level // 5) * 0.2
                    
                    # Define bases
                    base_sets = 1 if st.session_state.level < 5 else 2
                    base_reps = 10 if category == "Combat" else 5
                    base_mins = 5  # <--- ADD THIS
                    
                    scaled_sets = int(base_sets * level_multiplier)
                    scaled_reps = int(base_reps * level_multiplier)
                    scaled_mins = int(base_mins * level_multiplier) # <--- ADD THIS
                    # ---------------------------

                    with col1:
                        if category == "Combat":
                            sets = st.slider(f"Sets (Rounds)", 1, 10, scaled_sets, key=f"sets_{ex}")
                            reps = st.slider(f"Punches per set", 50, 500, scaled_reps * 20, key=f"reps_{ex}")
                        elif category == "Breathing" or category == "Yoga":
                            # Now scaled_mins is defined, so this won't crash
                            duration = st.slider(f"Minutes", 1, 60, scaled_mins, key=f"dur_{ex}")
                        else:
                            sets = st.slider(f"Sets", 1, 10, scaled_sets, key=f"sets_{ex}")
                            reps = st.slider(f"Reps", 5, 100, scaled_reps * 5, key=f"reps_{ex}")
                    
                    if col2.button(f"Log {ex}", key=f"btn_{ex}"):
                        # Logic to format the log based on what was selected
                        if category == "Combat":
                            effort_msg = f"{sets} rounds of {reps} punches"
                        elif category == "Breathing" or category == "Yoga":
                            effort_msg = f"{duration} minutes of {ex}"
                        else:
                            effort_msg = f"{sets} sets of {reps} reps"
                            
                        st.session_state.gallery_logs.append({
                            "date": datetime.now().strftime("%Y-%m-%d %H:%M"),
                            "caption": f"Completed {ex}: {effort_msg}"
                        })
